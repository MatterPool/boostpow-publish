...
/* NEW API CONFIGURATION OBJECT
    {
		...
		showTopic: true,
		topic: { // topic => if is a string or empty string show tag field with tag filled
			value: '',
...
export function normalizeLegacyApi(opts) {
	...
	newOpts.topic = normalizeTopic(opts);
...
function normalizeTopic(opts) {
	const defaults = { value: '', show: true, disabled: false };
	let o = undefined;

	if (opts.topic !== undefined) {
		if (opts.topic === '') return defaults;
		if (Helpers.hasStrLen(opts.topic, 1)) {
			// when simple string
			o = Object.assign({}, defaults, { value: opts.topic });
		} else if (
			Helpers.hasObjProp(opts.topic) && // when object
			(Helpers.hasStrLen(opts.topic.value, 0) || opts.topic.show === true) // with valid value or only show tag
		) {
			o = Object.assign({}, defaults, opts.topic);
		}
		else return o;
	} 
	if (Helpers.isStrictBool(opts.showTopic)) {
		o = Object.assign({}, defaults, { show: opts.showTopic });
		return o;
	}

	// previously field was called topic
